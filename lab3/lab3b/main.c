...
int main() {
    ...
    int (*func[])(Table *, FILE *, const char *err_msgs[]) = {NULL, D_Table_Add, D_Table_Single_Search,
    ...
    const char *err_msgs[] = {"Ok", "Overflow", "Error with input", "Existing Keys",
                              ...
                              "No elements with this key or release", "File's not exist"};

    Table *table = malloc(sizeof(*table));
    FILE *fTable;
    D_Table_Init(table, fTable, err_msgs);

    int choice = dialog(msgs, NMsgs, err_msgs);

    while (choice) {
        if (choice) {
            func[choice](table, fTable, err_msgs);
        }
        choice = dialog(msgs, NMsgs, err_msgs);
    }
    Table_Rewrite_Description(table, fTable);
    Table_Free(table);
    printf("Bye!\n");
    return 0;
}