...
int keySpace2Init(Table *table, int ms2, int cs2) {
    table->max_size2 = ms2;
    table->cur_size2 = cs2;
...
int ks2SearchVerKey(Table *table, SearchTab *new_table, FILE *fTable, char *key, int version, int choice) {
    if (choice == 1) {
        choice = 2;
    }
    else {
...
int ks2DeleteVerKey(Table *table,FILE * fTable, char *key, int version, int choice) {
    if (choice == 1) {
        choice = 2;
    } else {
        choice = 4;
