...
int keyspaceAdd(SearchTab *new_table,Table *table, FILE *fTable,  Item *tmp, int i) {
    ...
    new_table[i].key1 = key1;
    new_table[i].key2 = key2;
    new_table[i].element = data;
...
int addKeys(Table *table, char *key1, char *key2, int id2) {
    int release1;
    release1 = releaseKey1(table, key1);
    table->cur_size1++;
